...
class PlanManager extends Component
{
    ...
    public function executeConfirmedAction()
    {
        ...
        if ($this->confirmingAction === 'delete_rule') {
            \App\Models\PlanRule::destroy($this->confirmingId);
            $this->loadRules();
    ...
    public function loadRules()
    {
        ...
        $rules = \App\Models\PlanRule::with('zone')
            ->where('plan_id', $this->rulePlanId)
            ->orderBy('term_months')
            ->orderBy('rule_key')
            ->get();

        // Agregar info de herencia: cuántas zonas hijas heredan cada regla
    ...
    {
        if ($ruleId) {
            $rule = \App\Models\PlanRule::find($ruleId);
    ...
    public function saveRule()
    {
        ...
        if ($this->editingRuleId) {
            \App\Models\PlanRule::find($this->editingRuleId)->update([
            ...
            \App\Models\PlanRule::updateOrCreate(
                [
    ...
    {
        \App\Models\PlanRule::destroy($ruleId);
        $this->loadRules();
        $this->dispatch('show-toast', type: 'info', message: 'Regla eliminada.');
    }
    ...
    public function render()
    {
        ...
        if (in_array($this->activeTab, ['zones', 'rules'])) {
        ...
        // Cargar planes para selector de reglas
