    ...
    <div class="max-w-5xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-2">
                <x-ui.button variant="primary" icon="qr_code_scanner" href="{{ route('bodega.shipments.receive', ['code' => $shipment->code]) }}" class="!bg-green-600 hover:!bg-green-700">
                    Recibir
                </x-ui.button>
                <x-ui.button variant="secondary" href="{{ route('bodega.shipments.index') }}">Volver</x-ui.button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2">
                <x-ui.card title="Estado del envío" icon="timeline">
                    <div class="relative">
            ...
            <div class="space-y-4" x-data="{ showQr: false, showBarcode: false, copied: false }">
                <x-ui.card title="Código de envío" icon="qr_code">
                    <div class="relative mb-4">
                        <p class="text-lg font-mono font-bold text-gray-800 tracking-widest select-all bg-gray-50 px-3 py-2.5 rounded-lg border border-gray-200">
                            {{ $shipment->code }}
                        </p>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <button type="button" @click="showQr = true"
                            ...
                            <span class="material-symbols-outlined text-blue-600 text-2xl group-hover:scale-110 transition-transform">qr_code</span>
                            <span class="text-xs font-medium text-gray-700">Ver QR</span>
                        </button>
                        <button type="button" @click="showBarcode = true"
                            class="flex flex-col items-center gap-1.5 px-3 py-3.5 rounded-xl border-2 border-gray-200 hover:border-blue-400 hover:bg-blue-50 transition cursor-pointer group">
                            <span class="material-symbols-outlined text-blue-600 text-2xl group-hover:scale-110 transition-transform">barcode</span>
                    ...
                    <p class="text-[10px] text-gray-400 mt-3">Usá el QR, código de barras o copiá el código para confirmar</p>
                </x-ui.card>
                ...
                <div x-show="showBarcode" x-cloak x-transition:enter="ease-out duration-200"
                    ...
                    <div x-show="showBarcode" x-transition:enter="ease-out duration-200 delay-100"
                        ...
                        <div class="flex justify-center mb-4 overflow-x-auto">
                            ...
                            <div class="p-4 bg-white rounded-xl border-2 border-gray-200 shadow-sm inline-block min-w-[300px]">
                ...
                {{-- Items del envío --}}
                <x-ui.card title="Productos" icon="inventory_2">
                    <div class="space-y-2">
                        @foreach($shipment->items as $item)
                            <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                                ...
                                <span class="text-sm font-mono font-bold text-gray-900 bg-white px-3 py-0.5 rounded border border-gray-200">{{ (int) $item->quantity }}</span>
                            </div>
                        @endforeach
                    </div>
                </x-ui.card>
        ...
        {{-- Notas --}}
        @if($shipment->notes)
            <x-ui.alert variant="warning" icon="sticky_note_2" title="Notas del envío">
                {{ $shipment->notes }}
            </x-ui.alert>
        @endif
    </div>
