<div class="max-w-6xl mx-auto">
    <x-ui.card title="Nuevo traspaso entre sucursales" icon="swap_horiz" subtitle="Movimiento de material entre dos sucursales" overflow="visible">
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <x-ui.select wire:model.live="originBranchId" label="Sucursal origen" placeholder="Seleccioná..." icon="logout" required>
                    @foreach($branches as $b)
                    <option value="{{ $b->id }}">{{ $b->name }}</option>
                    @endforeach
                </x-ui.select>
                <x-ui.select wire:model.live="targetBranchId" label="Sucursal destino" placeholder="Seleccioná..." icon="login" required>
                    @foreach($branches as $b)
                    <option value="{{ $b->id }}">{{ $b->name }}</option>
                    @endforeach
                </x-ui.select>
            ...
            @if($originBranchId && $originBranchId === $targetBranchId)
                <x-ui.alert variant="danger">La sucursal de origen y destino no pueden ser la misma.</x-ui.alert>
            @endif

            <div class="border-t border-gray-200 pt-5">
                <x-forms.label icon="inventory_2">Producto a traspasar</x-forms.label>
            ...
            @if($selectedProductId)
                @if(!$originBranchId)
                    <x-ui.alert variant="warning">Seleccioná la sucursal de origen para ver el stock disponible.</x-ui.alert>
                @else
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="bg-blue-50 rounded-lg p-4 border border-blue-200">
                        <p class="text-xs text-blue-600 uppercase">Disponible en {{ $branches->firstWhere('id', (int) $originBranchId)?->name }}</p>
                        <p class="text-xl font-bold text-blue-900 font-mono">{{ $requiresDevice ? count($devices) : (int) $available }}</p>
                    </div>
                </div>
                ...
                <div class="bg-gray-50 rounded-xl border border-gray-200 p-4 space-y-3">
                    <div class="flex items-center justify-between">
                        <h3 class="text-sm font-semibold text-gray-700">Seleccionar dispositivos a traspasar</h3>
                ...
                <x-ui.input type="number" icon="inventory_2" wire:model="quantity" min="0" max="{{ (int) $available }}" label="Cantidad a traspasar" placeholder="0" />
                @endif
                ...
                <div class="flex justify-end gap-3 pt-4 border-t border-gray-200">
                    ...
                    <x-ui.button variant="primary" icon="swap_horiz" wire:click="save">Crear traspaso</x-ui.button>
                </div>
            @endif
        </div>
    </x-ui.card>
