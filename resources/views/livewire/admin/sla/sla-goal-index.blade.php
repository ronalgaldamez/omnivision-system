<div class="max-w-7xl mx-auto">
    <x-ui.card icon="timer" title="Metas SLA" subtitle="Configuración de tiempos objetivo por prioridad y tipo de servicio">
        <x-slot:headerActions>
            <x-ui.button variant="primary" icon="add_circle" href="{{ route('admin.sla.goals.create') }}">
                Nueva Meta SLA
            </x-ui.button>
        </x-slot:headerActions>

        <x-ui.input type="text" wire:model.live="search" placeholder="Buscar meta SLA..." icon="search" class="mb-5" />
                        ...
                        <tr class="hover:bg-gray-50/80 transition">
                            <td class="px-4 py-3">
                                @php
                                    ...
                                    $color = $colors[$goal->priority] ?? 'gray';
                                @endphp
                                <x-ui.badge variant="{{ $color }}">{{ $goal->priority }}</x-ui.badge>
                            </td>
