<div class="max-w-2xl mx-auto">
    <x-ui.card icon="edit" title="Editar Usuario" subtitle="Modifica los datos del usuario">
        <div class="space-y-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Sucursales que administra</label>
                    <select wire:model="managedBranches" multiple
                        class="w-full px-3 py-2.5 rounded-lg border border-gray-300 bg-white shadow-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition text-sm min-h-[90px]">
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}">{{ $branch->name }}</option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-400 mt-1">Si se eligen, el usuario podrá acceder solo a estas sucursales (aunque sean de otra empresa). Si queda vacío, usa la regla de su rol/sucursal.</p>
                    @error('managedBranches') <span class="text-xs text-red-500 mt-1 block">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Rol Técnico</label>
            ...
            <div class="flex justify-end gap-3">
                ...
                <x-ui.button variant="primary" icon="save" wire:click="save">Actualizar</x-ui.button>
            </div>
        </div>
    </x-ui.card>
            ...
            <div class="space-y-2">
                @foreach($tabModules as $tab)
                    @php
                        ...
                        if(!$permissionsPersonalized && !$has) {
                            $g = ['gates'=>[],'menus'=>[],'actions'=>[]];
                            $has = false;
                        }
                        $open = $activeTab === $tab;
                    @endphp
