            ...
            <div class="p-6">
                <form wire:submit="promptSave" class="space-y-6">
                    ...
                    <div>
                        ...
                        @error('name') <span class="text-xs text-red-600 mt-1.5 block font-medium">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="relative">
                                <select wire:model="document_type"
                                    class="w-full pl-10 pr-8 py-2.5 rounded-lg border border-gray-200 bg-gray-50 text-gray-900 text-sm focus:border-gray-400 focus:bg-white focus:outline-none transition appearance-none">
                        ...
                        <div>
                            ...
                            @error('document_number') <span
                                class="text-xs text-red-600 mt-1.5 block font-medium">{{ $message }}</span>
                            @enderror
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <label for="accepts_promotions" class="text-sm text-gray-700 cursor-pointer">Acepta recibir
                            promociones o facturas electrónicas</label>
                    </div>

                    <!-- Departamento, Municipio, Distrito (cascada desde zonas) -->
                    <div class="pb-5 border-b border-gray-100">
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div>
                                <label
                                    class="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">Distrito
                                    / Localidad</label>
                    ...
                    <div class="flex items-center gap-2 py-2">
                        <input type="checkbox" wire:model.live="no_price" id="no_price"
                            class="w-4 h-4 rounded border-gray-300 text-gray-700 focus:ring-gray-500">
                        <label for="no_price" class="text-sm text-gray-700 cursor-pointer font-medium">Sin
                            precio</label>
                    </div>
                    @if($no_price)
                        <p class="text-xs text-gray-500 -mt-3 mb-4 italic">Nota: más adelante se le puede asignar el paquete
                            al cliente.
                        </p>
                    @endif
                    ...
                    @if(!$no_price)
                        <div class="bg-gray-50 rounded-lg border border-gray-200 p-4 space-y-4">
                            <h3 class="text-xs font-bold text-gray-800 uppercase tracking-wide flex items-center gap-2">
                                ...
                                Servicio contratado
                                <span
                                    class="text-xs font-normal text-gray-500 normal-case tracking-normal">(opcional)</span>
                            </h3>
                            ...
                            <div>
                                <label
                                    class="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">Plan</label>
                                    ...
                                    <div class="flex items-start justify-between">
                                        ...
                                        <div class="text-right">
                                            <p class="text-lg font-bold text-gray-900">
                                                ${{ number_format($selectedPlanPrice, 2) }}</p>
                                            @if($selPlan['base_price'] != $selPlan['price'])
                                                <p class="text-xs text-gray-500 mt-0.5">Base:
                                                    ${{ number_format($selPlan['base_price'], 2) }}</p>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                ...
                                @foreach($contractHistory as $entry)
                                    <div class="px-4 py-3 flex items-center justify-between text-sm">
                                        <div class="flex items-center gap-3 min-w-0">
                                            <span
                                                class="material-symbols-outlined text-base text-gray-400 flex-shrink-0">assignment</span>
                                            <div class="min-w-0">
                                                <p class="font-medium text-gray-800 truncate">{{ $entry['plan_name'] }}</p>
                                                <p class="text-xs text-gray-500">{{ $entry['zone_name'] }} —
                                                    {{ $entry['date'] }}
                                                </p>
                                            </div>
                                        </div>
                                        <div class="flex items-center gap-3 flex-shrink-0">
                                            @if($entry['price'])
                                                <span
                                                    class="text-sm font-semibold text-gray-900">${{ number_format($entry['price'], 2) }}</span>
                                            @endif
                                            <span
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-gray-100">
                        ...
                        <button type="button" wire:click="promptClear"
                            class="px-6 py-2.5 border border-red-300 rounded-lg text-sm font-semibold text-red-700 bg-white hover:bg-red-50 focus:outline-none transition inline-flex items-center gap-2">
                ...
                <div class="bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
                    <div class="p-6 text-center">
                        ...
                        <p class="text-sm text-gray-600 mt-2">
                            {{ $clientId ? '¿Guardar los cambios del cliente?' : '¿Registrar este nuevo cliente?' }}
                        </p>
                    </div>
                    <div class="bg-gray-50 px-6 py-4 flex flex-col gap-3 sm:flex-row-reverse">
                        <button type="button" wire:click="executeSave"
                            class="w-full sm:w-auto px-6 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 focus:outline-none transition inline-flex items-center justify-center gap-2">
                            <span class="material-symbols-outlined text-base">check</span>
                            Sí, continuar
                        </button>
                        <button type="button" @click="open = false" wire:click="cancelSave"
                            class="w-full sm:w-auto px-6 py-2.5 bg-white border border-gray-300 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-50 focus:outline-none transition">
                            Cancelar
                        </button>
                    </div>
                </div>
            ...
            <div class="relative mx-auto p-5 w-full max-w-md">
                <div class="bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
                    <div class="p-6 text-center">
                        ...
                        <p class="text-sm text-gray-600 mt-2">¿Estás seguro de limpiar todos los campos? Se perderán los
                            datos ingresados.</p>
                    </div>
                    <div class="bg-gray-50 px-6 py-4 flex flex-col gap-3 sm:flex-row-reverse">
                        <button type="button" wire:click="executeClear"
                            class="w-full sm:w-auto px-6 py-2.5 bg-red-600 text-white rounded-lg text-sm font-semibold hover:bg-red-700 focus:outline-none transition inline-flex items-center justify-center gap-2">
                            <span class="material-symbols-outlined text-base">delete_sweep</span>
                            Sí, limpiar
                        </button>
            ...
            <div class="bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden">
                ...
                <div class="bg-gray-50 px-6 py-4 flex flex-col gap-3 sm:flex-row-reverse">
                    <button type="button" wire:click="executeCancel"
                        class="w-full sm:w-auto px-6 py-2.5 bg-gray-600 text-white rounded-lg text-sm font-semibold hover:bg-gray-700 focus:outline-none transition inline-flex items-center justify-center gap-2">
