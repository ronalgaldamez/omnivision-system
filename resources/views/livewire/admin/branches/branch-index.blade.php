<div>
    <div class="flex items-center justify-between mb-5">
        <div>
            <h3 class="text-base font-semibold text-gray-900 flex items-center gap-2">
                <span class="material-symbols-outlined text-gray-500 text-xl">store</span>
                Sucursales
            </h3>
            <p class="text-xs text-gray-500 mt-0.5">Sucursales operativas vinculadas a una empresa</p>
        </div>
    ...
    <x-ui.input type="text" wire:model.live="search" placeholder="Buscar por nombre, código o empresa..." icon="search" class="mb-5" />
        ...
        <table class="min-w-full text-sm">
            <thead>
                <tr class="bg-gray-50 border-b border-gray-200">
                    ...
                    <th class="px-4 py-3 text-left text-gray-600 font-medium">
                        <div class="flex items-center gap-1.5">
                            <span class="material-symbols-outlined text-gray-400 text-base">apartment</span>
                            Empresa
                        </div>
                    </th>
                    <th class="px-4 py-3 text-left text-gray-600 font-medium">
            ...
            <tbody class="divide-y divide-gray-100">
                @forelse($branches as $branch)
                    <tr class="hover:bg-gray-50/80 transition {{ !$branch->is_active ? 'opacity-60' : '' }}">
                        ...
                        <td class="px-4 py-3 text-gray-600 text-xs">{{ $branch->company?->displayName() ?? '—' }}</td>
