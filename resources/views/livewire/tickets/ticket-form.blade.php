    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200/80 overflow-hidden">
        ...
        <div class="p-6">
            ...
            <form wire:submit.prevent="promptSave" class="space-y-6">
                ...
                {{-- ========== ZONA Y PLAN ========== --}}
                @if($selectedClient)
                <div class="bg-gray-50/50 rounded-xl border border-gray-200 p-5 space-y-4">
                    <h2 class="text-sm font-semibold text-gray-800 flex items-center gap-2">
                        <span class="material-symbols-outlined text-gray-500 text-base">map</span>
                        Contratación
                    </h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1.5">Zona de cobertura</label>
                            <select wire:model.live="zone_id" {{ $editingEnabled ? '' : 'disabled' }}
                                class="w-full px-3 py-2.5 rounded-lg border border-gray-300 bg-white text-sm disabled:bg-gray-100">
                                <option value="">Seleccione zona</option>
                                @foreach($availableZones as $zone)
                                    @if($zone->level === 'localidad')
                                    <option value="{{ $zone->id }}">
                                        {{ $zone->parent?->name ? $zone->parent->name.' > ' : '' }}{{ $zone->name }}
                                    </option>
                                    @endif
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1.5">Plan contratado</label>
                            <select wire:model.live="plan_id" {{ $editingEnabled ? '' : 'disabled' }}
                                class="w-full px-3 py-2.5 rounded-lg border border-gray-300 bg-white text-sm disabled:bg-gray-100">
                    ...
                    @if($selectedPlanPrice !== null && $plan_id)
                    <div class="flex items-center gap-2 text-sm">
                        <span class="text-gray-500">Precio:</span>
                        <span class="font-semibold text-gray-800">${{ number_format($selectedPlanPrice, 2) }}</span>
                        @php
                            $plan = \App\Models\Plan::find($plan_id);
                        @endphp
                        @if($plan && $selectedPlanPrice != $plan->base_price)
                            <span class="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">
                                Precio base: ${{ number_format($plan->base_price, 2) }}
                            </span>
                        @endif
                        <span class="text-xs text-gray-400">(precios pueden variar según la zona)</span>
                    </div>
                    @endif
                </div>
                @endif
